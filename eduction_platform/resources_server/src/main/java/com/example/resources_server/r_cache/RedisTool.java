...
import com.example.resources_server.entity.LearningMaterials;
...
public class RedisTool<T> {
     ...
     */
    public void openTransaction(Map<LearningMaterials, Method> methodMap) throws IllegalAccessException{
        stringRedisTemplate.setEnableTransactionSupport(true);
        stringRedisTemplate.multi();
        methodMap.keySet().stream().forEach(key->{
            try {
                if (methodMap.get(key).getName().contains("set")){
                    methodMap.get(key).invoke(stringRedisTemplate,key.getId(),key);
                }else {
                    methodMap.get(key).invoke(stringRedisTemplate,key.getId());
                }
