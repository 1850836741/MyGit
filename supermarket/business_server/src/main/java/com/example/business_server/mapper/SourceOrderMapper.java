...
public interface SourceOrderMapper {
    ...
    SourceOrder findSourceById(@Param(value = "source_order_id") String source_order_id);

    /**
     * 查询对应时间的货源订单
     * @param source_order_time
     * @return
     */
    @Select("SELECT * FROM source_order WHERE source_order_time LIKE "+"'%"+"#{source_order_time}"+"%'")
    List<SourceOrder> findSourceLimitTime(@Param("source_order_time") String source_order_time);
    ...
    void addSource(@Param(value = "sourceOrder") SourceOrder sourceOrder);
    ...
    List<SourceOrderInformation> findSourceInformationById(@Param(value = "source_order_id") String source_order_id);
     ...
     * @return
     */
    @Select("SELECT * FROM source_order LIMIT 0,#{number}")
    List<SourceOrder> findSourceLimitNumber(@Param(value = "number") long number);
